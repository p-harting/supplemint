...
def update_on_save(sender, instance, created, **kwargs):
    """
    Signal to update order total and product stock when a line item is created or updated.
    - If the line item is newly created, update the stock for the product or product size.
    ...
    if created:
        ...
        if instance.product_size:
            ...
            product_size.stock -= instance.quantity  # Decrease stock for the specific size
            product_size.save()
        else:
            product.stock -= instance.quantity  # Decrease stock for the product itself
            product.save()
...
def update_on_delete(sender, instance, **kwargs):
    """
    Signal to update order total and restore product stock when a line item is deleted.
    ...
    if instance.product_size:
        product_size = product.sizes.get(name=instance.product_size)
        product_size.stock += instance.quantity  # Restore stock for the specific size
        product_size.save()
    else:
        product.stock += instance.quantity  # Restore stock for the product itself
        product.save()
